...
CAP_HOURS = float(os.environ.get("MAX_GPU_HOURS_STAGE_B0E", "4"))
DEFAULT_EVAL_SAMPLES = int(os.environ.get("B0E_EVAL_SAMPLES", "8192"))
RESULTS_CSV = RUN_DIR / "results" / "stage_b0e_results.csv"
...
def b0e1_early_stop(metrics: dict[str, dict[int, dict[str, float]]], status: str) -> bool:
...
def b0e2_early_stop(metrics: dict[str, dict[int, dict[str, float]]], status: str, n_values: int) -> bool:
    induction = metrics["induction_capped"]
    return (
        status not in {"fail_non_finite_loss", "fail_grad_norm", "budget_pause_actual"}
        and induction.get(1, {}).get("accuracy", 0.0) >= 0.99
    ...
    *,
    ...
    for step in range(start_step + 1, cfg.max_steps + 1):
        ...
        if (time.time() - stage_start_time) / 3600.0 > CAP_HOURS:
            ...
            notes = f"actual Stage B0E elapsed exceeded cap {CAP_HOURS:.2f} GPU-hours"
            break
...
def classify_b0e1(outcomes: list[dict[str, Any]]) -> str:
...
def classify_b0e2(outcome: dict[str, Any] | None, n_values: int) -> str:
    if outcome is None:
        return "SKIPPED"
    induction = outcome.get("finals", {}).get("induction_capped", {})
...
def run_sampler_audit() -> bool:
    validate_phases(B0E1_PHASES)
    validate_phases(B0E2_PHASES)
    ...
    def sample_fps(with_eval: bool) -> tuple[list[str], list[int]]:
        ...
        for step in range(1, 41):
            phase = phase_for_step(B0E1_PHASES, min(step * 750, 30000))
    ...
    phase_examples = {
        str(step): list(phase_for_step(B0E2_PHASES, step).train_n_pairs)
        for step in [1, 5000, 5001, 10000, 10001, 20000, 20001, 35000, 35001, 50000]
    }
    lines = ["# Stage B0E Sampler Audit\n\n"]
    ...
    lines.append("- B0E default: q_count_mode=capped, q_cap=8\n\n")
    lines.append("## Curriculum Phases\n\n")
    lines.append("### B0E.1\n\n```json\n" + json.dumps([phase.as_jsonable() for phase in B0E1_PHASES], indent=2) + "\n```\n\n")
    lines.append("### B0E.2\n\n```json\n" + json.dumps([phase.as_jsonable() for phase in B0E2_PHASES], indent=2) + "\n```\n\n")
    ...
    write_text(RUN_DIR / "summaries" / "stage_b0e_sampler_audit.md", "".join(lines))
    return rng_ok
...
def plot_curves() -> None:
    ...
    for path in sorted((RUN_DIR / "results").glob("stage_b0e_*.csv")):
        if path.name == "stage_b0e_results.csv":
            continue
    ...
    fig.tight_layout()
    fig.savefig(RUN_DIR / "plots" / "stage_b0e_induction_aligned_learning_curves.png", dpi=160)
    fig.savefig(RUN_DIR / "plots" / "stage_b0e_induction_aligned_learning_curves.pdf")
    plt.close(fig)
...
def write_budget_pause(estimated_hours: float, actual_hours: float, reason: str) -> None:
    write_text(
        RUN_DIR / "summaries" / "stage_b0e_budget_pause.md",
        f"""# Stage B0E Budget Pause
...
def write_cost_update(outcomes_b0e1: list[dict[str, Any]], outcome_b0e2: dict[str, Any] | None, b0e1_gate: str, b0e2_gate: str) -> None:
    b0e1_sec = measured_sec_per_step(outcomes_b0e1)
    b0e2_sec = measured_sec_per_step([outcome_b0e2] if outcome_b0e2 else [])
    best_sec = b0e2_sec or b0e1_sec or prior_sec_per_step()
    b1_steps = 3 * 20000 + 80000
    b2_steps = 2 * 100000
    stage_b_hours = (b1_steps + b2_steps) * best_sec / 3600.0
    actual_hours = sum(out.get("elapsed_sec", 0.0) for out in outcomes_b0e1 + ([outcome_b0e2] if outcome_b0e2 else [])) / 3600.0
    n64_seq = max_induction_seq_len(64, "capped", 8, 0)
    old_ans_slot_n64_seq = 1 + 4 * 64 + 4 * 8 - 1
    lines = ["# Stage B0E Cost Update\n\n"]
    lines.append(f"- Measured sec/step for B0E.1: {b0e1_sec:.5f}\n" if b0e1_sec else "- Measured sec/step for B0E.1: unavailable\n")
    lines.append(f"- Measured sec/step for B0E.2: {b0e2_sec:.5f}\n" if b0e2_sec else "- Measured sec/step for B0E.2: not run\n")
    lines.append(f"- Actual Stage B0E elapsed GPU-hours: {actual_hours:.3f}\n")
    lines.append(f"- Estimated full Stage B cost under key_next_value format: {stage_b_hours:.3f} GPU-hours; one model covers all eval_n_pairs.\n")
    lines.append("- Estimated full Stage C cost under same recipe: unavailable until Mamba induction sequence throughput probe.\n")
    lines.append("- Mamba throughput probe should be refreshed: YES, if Stage B becomes allowed, because the token format changed.\n")
    lines.append(f"- Sequence length reduction versus old <V>/<ANS> format: old N=64 q_cap=8 x length {old_ans_slot_n64_seq}; key_next_value x length {n64_seq}.\n")
    lines.append(f"- Current B0E cap likely sufficient: {'YES' if actual_hours <= CAP_HOURS else 'NO'}\n")
    lines.append(f"- B0E.1 gate: {b0e1_gate}\n")
    lines.append(f"- B0E.2 gate: {b0e2_gate}\n")
    write_text(RUN_DIR / "summaries" / "stage_b0e_cost_update.md", "".join(lines))
...
def table_rows() -> str:
    ...
    for row in rows:
        lines.append(
            f"| {row.get('substage','')} | {row.get('eval_mode','')} | {row.get('task_format','')} | {row.get('n_values','')} | {row.get('q_count_mode','')} | {row.get('q_cap','')} | {row.get('lr','')} | {row.get('seed','')} | {row.get('step','')} | "
...
def write_summary(
    *,
    ...
    stabilized = "YES" if b0e1_gate in {"PASS", "PASS_STRONG"} else "NO"
    flexible = "YES" if b0e1_gate in {"PASS", "PASS_STRONG"} else "NO"
    transfer = "YES" if b0e1_gate in {"PASS", "PASS_STRONG"} else "NO"
    formal_viable = "YES" if b0e2_gate in {"PASS", "PASS_STRONG"} else "NOT YET" if b0e2_gate == "PARTIAL_RISING" else "NO"
    summary = f"""# Stage B0E Summary

## Result

- Sampler audit: {'PASS' if sampler_ok else 'FAIL'}
- B0E.1 N_VALUES=8 induction-aligned reproducibility: {b0e1_gate}
- B0E.2 N_VALUES=16 formal pilot: {b0e2_gate}
...
1. Did induction multi-query supervision stabilize N=8 across seeds? {stabilized}.
2. Did the model learn a flexible recall rule across N=1/2/4/8? {flexible}.
3. Does the induction-trained model transfer to single-query eval? {transfer}.
4. Is formal N_VALUES=16 training viable? {formal_viable}.
5. Should the formal task be redefined as key_next_value recall? {'YES' if formal_task == 'key_next_value' else 'NO'}.
...
- {recommended_next}
"""
    write_text(RUN_DIR / "summaries" / "stage_b0e_summary.md", summary)
...
def main() -> None:
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required for Stage B0E")
    log(f"[stage_b0e] run_dir={RUN_DIR} commit={git_hash()} cap_hours={CAP_HOURS:.2f}")
    ...
    if not sampler_ok:
        write_summary(
            ...
            recommended_next="increase Transformer control size",
        )
        return
    ...
    if initial_est > CAP_HOURS:
        write_budget_pause(initial_est, 0.0, "estimated B0E.1 cost exceeds cap")
        write_cost_update([], None, "FAIL", "SKIPPED")
        write_summary(
            sampler_ok=True,
            b0e1_gate="FAIL",
            b0e2_gate="SKIPPED",
            ...
            stage_b_recipe="do not proceed",
            full_stage_b_allowed="NO",
            recommended_next="extend to 100k with user confirmation",
        )
        return

    eval_modes = (("induction_capped", "capped", 8), ("single_query", "single", 1))
    outcomes_b0e1: list[dict[str, Any]] = []
    b0e1_pairs = (1, 2, 4, 8)
    for seed in [0, 1, 2]:
        run_id = f"stage_b0e_b01_induction_v8_seed{seed}"
        cfg = make_cfg(run_id, seed, 8, b0e1_pairs, 30000, 1000, 192, 1e-3)
        append_manifest(
            f"B0E_1_induction_v8_seed{seed}",
            cfg,
        ...
        outcome = train_induction_run(
            substage="B0E.1",
            run_id=run_id,
            recipe="induction capped curriculum N_VALUES=8",
            cfg=cfg,
            phases=B0E1_PHASES,
            eval_n_pairs=b0e1_pairs,
            eval_modes=eval_modes,
            eval_seeds=(0, 1),
            eval_samples_per_condition=DEFAULT_EVAL_SAMPLES,
            eval_batch_size=192,
            checkpoint_interval=1000,
            early_stop_fn=b0e1_early_stop,
            consecutive_passes=2,
            stage_start_time=stage_start,
        )
        outcomes_b0e1.append(outcome)
    ...
    b0e1_gate = classify_b0e1(outcomes_b0e1)
    if b0e1_gate == "FAIL":
        write_cost_update(outcomes_b0e1, None, b0e1_gate, "SKIPPED")
        write_summary(
            sampler_ok=True,
            b0e1_gate=b0e1_gate,
            b0e2_gate="SKIPPED",
        ...
        log("[stage_b0e] stop after B0E.1 FAIL")
        return

    sec_after_b0e1 = measured_sec_per_step(outcomes_b0e1) or base_sec
    b0e2_est = 50000 * sec_after_b0e1 * 2.6 / 3600.0
    if not enough_budget_for(b0e2_est, stage_start):
        write_budget_pause(b0e2_est, (time.time() - stage_start) / 3600.0, "estimated B0E.2 cost exceeds remaining cap")
        write_cost_update(outcomes_b0e1, None, b0e1_gate, "SKIPPED")
        write_summary(
            sampler_ok=True,
            b0e1_gate=b0e1_gate,
    ...
    b0e2_pairs = (1, 2, 4, 8, 16, 32, 64)
    cfg2 = make_cfg("stage_b0e_b02_induction_v16_seed0", 0, 16, b0e2_pairs, 50000, 2000, 96, 1e-3)
    append_manifest(
        "B0E_2_induction_v16_seed0_lr1e-3",
        cfg2,
    ...
    outcome_b0e2 = train_induction_run(
        substage="B0E.2",
        run_id=cfg2.run_id,
        recipe="formal induction capped curriculum N_VALUES=16",
        cfg=cfg2,
        phases=B0E2_PHASES,
        eval_n_pairs=b0e2_pairs,
        eval_modes=eval_modes,
        eval_seeds=(0, 1),
        eval_samples_per_condition=DEFAULT_EVAL_SAMPLES,
        eval_batch_size=96,
        checkpoint_interval=2000,
        early_stop_fn=lambda metrics, status: b0e2_early_stop(metrics, status, 16),
        consecutive_passes=2,
        stage_start_time=stage_start,
    )
    append_stage_results(outcome_b0e2, cfg2)
    append_manifest(
        "B0E_2_induction_v16_seed0_lr1e-3",
        cfg2,
        "curriculum to 64;q_count_mode=capped;q_cap=8",
        "1,2,4,8,16,32,64;eval_modes=induction_capped,single_query",
        outcome_b0e2["elapsed_sec"] / 3600.0,
        outcome_b0e2["status"],
    )
    plot_curves()
    b0e2_gate = classify_b0e2(outcome_b0e2, 16)

    if b0e2_gate in {"PASS", "PASS_STRONG"}:
        full_allowed = "YES"
        formal_task = "key_next_value"
        stage_b_recipe = "induction-aligned curriculum"
        recommended = "Enter full Stage B with induction-aligned curriculum."
    elif b0e2_gate == "PARTIAL_RISING":
        full_allowed = "NO"
        formal_task = "do not proceed"
        stage_b_recipe = "do not proceed"
        recommended = "Ask user whether to extend B0E.2 to 100k."
    else:
        full_allowed = "NO"
        formal_task = "do not proceed"
        stage_b_recipe = "do not proceed"
        recommended = "Do not enter Stage B; reduce difficulty, increase control size, or auxiliary pretraining."

    write_cost_update(outcomes_b0e1, outcome_b0e2, b0e1_gate, b0e2_gate)
    write_summary(
        sampler_ok=True,
        b0e1_gate=b0e1_gate,
        b0e2_gate=b0e2_gate,
    ...
    log(f"[stage_b0e] done B0E.1={b0e1_gate} B0E.2={b0e2_gate} allowed_B={full_allowed}")
